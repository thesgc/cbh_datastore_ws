...
def step(context):
    ...
    result = context.api_client.get(
        "/dev/api/datastore/cbh_projects_with_forms", format="json")
    context.test_case.assertValidJSON(result.content)
    try:
        has_no_viewer_project = result.content.index(
            '"/dev/api/datastore/cbh_projects_with_forms/4"')
...
def step(context):
    ...
    result = context.api_client.get(
        "/dev/api/datastore/cbh_datapoint_classifications_nested?full=true", format="json")
    context.test_case.assertValidJSON(result.content)
...
def step(context):
    ...
    try:
        has_no_viewer_project = result.content.index(
            '"/dev/api/datastore/cbh_projects_with_forms/5"')
        context.test_case.assertTrue(True)
    except ValueError:
        # Exception should throw
    ...
    try:
        has_no_viewer_project = result.content.index(
            '"/dev/api/datastore/cbh_projects_with_forms/3"')
        context.test_case.assertTrue(True)
    except ValueError:
        # Exception should throw
        # print (result.content)
...
def step(context):
    post_data = {"data_form_config": "/dev/api/datastore/cbh_data_form_config/5",
                 "l0": "/dev/api/datastore/cbh_datapoints/2",
                 "l0_permitted_projects": ["/dev/api/datastore/cbh_projects_with_forms/3"],
                 "l1": {"project_data": {"TEST KEY": "TEST VALUE"}, "custom_field_config": "/dev/api/datastore/cbh_custom_field_config/96"}}
...
def step(context):
    post_data = {"data_form_config": "/dev/api/datastore/cbh_data_form_config/5",
                 "l0": "/dev/api/datastore/cbh_datapoints/2",
                 "l0_permitted_projects": ["/dev/api/datastore/cbh_projects_with_forms/5"],
                 "l1": {"project_data": {"TEST KEY": "TEST VALUE"}, "custom_field_config": "/dev/api/datastore/cbh_custom_field_config/96"}}
    created = context.api_client.post("/dev/api/datastore/cbh_datapoint_classifications",
                                      format="json",
                                      data=post_data)
...
def step(context):
    post_data = {"data_form_config": "/dev/api/datastore/cbh_data_form_config/5",
                 "l0": "/dev/api/datastore/cbh_datapoints/2",
                 "l0_permitted_projects": ["/dev/api/datastore/cbh_projects_with_forms/4"],
                 "l1": {"project_data": {"TEST KEY": "TEST VALUE"}, "custom_field_config": "/dev/api/datastore/cbh_custom_field_config/96"}}
    created = context.api_client.post("/dev/api/datastore/cbh_datapoint_classifications",
                                      format="json",
                                      data=post_data)
...
def step(context):
    classif = context.api_client.post("/dev/api/datastore/cbh_queries/_search?from=0&size=10",
...
def step(context):
    '''Here we are checking that the children data is available via the nested call'''
    result = context.api_client.get(
        "/dev/api/datastore/cbh_datapoint_classifications_nested?full=true&parent_id=None", format="json")
    context.test_case.assertValidJSON(result.content)
    data = json.loads(result.content)
    found_non_l0 = False
    print (data)
